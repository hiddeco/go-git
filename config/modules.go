...
// Git v2.54.0 check_submodule_name (submodule-config.c:214-237) rejects empty
// names and literal parent components. go-git additionally rejects dot and
// periods-only components, parent disguises, NULs, leading/trailing separators
// and drive prefixes. Both separators are recognized on every host.
// C Git accepts some of these extra names; the periods-only restriction is
// policy, not an assertion of NTFS parent folding.
