...
// TestReferenceNameRejectsDotsOnlyComponent pins a component of
// periods alone as refused, which the disguise predicate alone does
// not cover. ReferenceName.IsSafe rejects only a literal "." or
// "..", and validReferenceName is the only gate SetRef, Ref,
// RemoveRef and ReflogWriter pass through — ReferenceName.Validate,
// which refuses "..." under check_refname_format rules 3 and 7, is
// never called here. The refusal is go-git's own storage-name policy
// rather than a claim that NTFS folds "..." to a parent: Win32 trims
// a trailing period run only from a component that ends in one, so
// "..." canonicalises to the empty string, not to "..".
