...
// https://github.com/sigstore/sigstore/blob/6c7cc8d4d77676cd9f207cb3cfcdc054732d0dcb/pkg/signature/ssh/sign_test.go
//
...
var (
	// Generated with "ssh-keygen -C test@example.com -f id_rsa"
...
`
	// Generated with "ssh-keygen -C other-test@example.com -f id_rsa"
...
-----END OPENSSH PRIVATE KEY-----
`
	ed25519PublicKey = `ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEHjnNEfE88W1pvBLdV3otv28x760gdmPao3lVD5uAt9 test@example.com
`
)
...
func TestFromOpenSSH(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			run(t, nil, "ssh-keygen", "-Y", "sign", "-n", gitNamespace, "-f", privPath, dataPath)
...
func TestToOpenSSH(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			allowedSigner := "test@example.com " + tt.pub + "\n"
			allowedSigner += "othertest@example.com " + otherSSHPublicKey + "\n"
