...
var (
	...
	// their signatureType.
	knownSignatureFormats = signatureFormats{
...
// signatureFormats represents a list of signature formats, indexed by name.
type signatureFormats map[signatureType]signatureFormat

// typeForSignature returns the type of the signature based on its format.
func (s signatureFormats) typeForSignature(b []byte) signatureType {
	for t, i := range s {
		for _, begin := range i {
...
func parseSignedBytes(b []byte) (int, signatureType) {
	...
	for n < len(b) {
		var i = b[n:]
		if st := knownSignatureFormats.typeForSignature(i); st != signatureTypeUnknown {
			match = n
			t = st
		}
