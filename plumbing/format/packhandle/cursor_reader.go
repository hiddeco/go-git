...
//     via the underlying billy.ReaderAtCloser.
...
// mmap-backed billy.ReaderAtCloser, that unmaps the region the
// in-flight ReadAt is still reading from, which is a SIGBUS-class
// hazard. The caller must serialise Close with its own outstanding
// ReadAts on the same cursorReader.
//
